...
import { ChevronRight, ChevronLeft, Download, LogOut, ArrowLeft, QrCode } from 'lucide-react';
...
function DualCertificate() {
  ...
  const handleGenerateQR = () => {
    if (qrUrl && showRight) {
      ...
      alert("QR not available. Please click 'Left PDF' first to generate QR code.");
    } else if (!showRight) {
  ...
  const handleDownloadRight = () => {
    ...
    if (qrVisible && qrUrl) {
      ...
      qrImg.onload = () => {
        ...
        tempCtx.drawImage(qrImg, qrPosition.x, qrPosition.y, qrSize.width, qrSize.height);
  ...
  const handleMouseMove = (e) => {
    ...
    if (isResizing) {
      const newWidth = Math.max(60, Math.min(250, resizeStart.width + (e.clientX - resizeStart.x)));
      const newHeight = Math.max(60, Math.min(250, resizeStart.height + (e.clientY - resizeStart.y)));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex items-center justify-between px-4 py-4 bg-[#0b2545] text-white shadow-md min-h-[70px]">
        ...
        <div className="flex gap-4 items-center">
          ...
          <button onClick={() => navigate('/candidate-details')} className="hover:underline flex items-center gap-1 text-sm">
      ...
      <div className="flex flex-col items-center gap-6 py-10 px-4">
        <div className="flex items-center transition-all duration-700">
          <canvas ref={canvasLeftRef} width={595} height={842} className="border shadow-md"></canvas>

          {/* Right canvas + QR */}
          <div className={`relative transition-all duration-700 overflow-hidden ${showRight ? 'w-[595px] ml-6 opacity-100 translate-x-0' : 'w-0 opacity-0 translate-x-10'}`}>
            <canvas ref={canvasRightRef} width={595} height={842} className={`border shadow-md ${showRight ? 'block' : 'hidden'}`}></canvas>
